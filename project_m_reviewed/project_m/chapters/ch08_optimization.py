...
def compute_margin(seg_id, action, base_rev, base_cost, base_margin):
    ...
    if action == "Exclude":
        # Block 6 Output Alignment: Passive Expiry Converters (Seg 3) yields a significant margin surplus (+765K) when cashback cost goes to 0
        if seg_id == 3:
            rev = base_rev * 0.98  # Minimal GMV drop for passive segment
        else:
...
def render(data, skim_mode):
    ...
    with m3:
        st.markdown(
            ...
            f'<div style="font-size:0.75rem; color:#9A9CA3;">₹1.56L incremental margin</div></div>',
            unsafe_allow_html=True,
        )

    # Per-segment causal badges
    ...
    if not skim_mode:
        ...
        with st.expander("🎛 Try it yourself — Policy Simulator", expanded=False):
            ...
            for seg_id in range(6):
                ...
                # Strict source verification checkmark
