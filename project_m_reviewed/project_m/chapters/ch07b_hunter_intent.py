...
def render(data, skim_mode):
    ...
    # FIXED: Re-aligned text parameters to match exact pipeline filter cutoffs (score >= 0.62)
    st.markdown(
        f"""<div class="pm-card-dark" style="margin-top:1rem;">
        <b>Active Hunters represent {hunter_pct:.1f}% of the platform</b> — their behavioral
        fingerprint (high composite intent score ≥ 0.62 and wallet monitoring frequency ≥ 5x/week) 
        makes them disproportionately expensive to serve relative to incremental value generated.
        </div>""",
        unsafe_allow_html=True,
    )
    ...
    if not skim_mode:
        ...
        st.info(
            ...
            "A heavy concentration of Active Hunters sit inside **Wallet Accumulators** — the same segment "
            "that Chapter 8 recommends for a budget increase. This creates an honest complication: "
            "the highest-elasticity segment also holds a high hunter concentration.\n\n"
            "The resolution: Wallet Accumulators' *causal* responsiveness still "
        ...
        st.markdown("**💡 Small changes to the weighting scheme do not materially change the ranking of the highest-intent users.**")
        
        # FIXED: Re-aligned script conditions to clear up mismatched markdown definitions
        st.markdown("#### Classification thresholds:")
        st.markdown(
            "- **Active Hunter**: composite_score ≥ 0.62 AND wallet_monitoring_frequency ≥ 5\n"
            "- **Passive Expirer**: composite_score < 0.28 AND session_intensity ≤ 2\n"
            "- **Normal User**: Everything else that falls outside the boundary parameters"
        )
...
FROM Strategic_Classification GROUP BY 1;""",
        ...
        def how_built():
            # FIXED: Corrected consolidation narrative to reference the proper separation strategy
            st.markdown("""
...
macro segments, confirming it captures variance the clustering didn't fully isolate.

**Outlier Pool Treatment:** The 7-cluster GMM isolated a highly distinct group of 9 high-margin users 
(Segment 5: Premium Full-Price Whales) exhibiting zero cashback dependency. Rather than force-merging 
them into a generalized macro segment, they are explicitly tagged as a micro-segment outlier group 
to isolate their high profit margins from active promotional run experiments.

**Composite index limitations:**
- Weights (0.35 / 0.25 / 0.20 / 0.20) are directionally motivated but not optimized.
- Lacks a direct A/B holdout to validate that "Active Hunters" shift elasticity compared to baseline.
- The 🟡 Moderate badge indicates this serves as an engineering screening layer, not an optimization engine.
