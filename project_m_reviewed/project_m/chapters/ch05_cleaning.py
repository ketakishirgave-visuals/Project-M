...
CLEANING_ISSUES = [
    ...
    },
    {
        ...
        "fix": "Filled with 'Unknown' — preserves record completeness without introducing demographic bias through arbitrary imputation.",
    ...
    },
    {
        ...
        "result": "0 timeout-pattern nulls in this run · structural nulls kept intact. Imputing these values would fabricate reward issuance that never occurred.",
...
def render(data, skim_mode):
    ...
    question_answer(
        ...
        answer="All identified anomalies were classified, documented, and either corrected or intentionally preserved with a complete audit trail.",
        badge_level="strong",
    )
    ...
    st.warning(
        ...
        "- **The `deliberately_expired` flag** — This represents an intentionally simulated behavioural outcome where a customer allows cashback to expire. It is behavioural signal, not missing or corrupted data.\n"
...
# 1. Locate where this chapter file is sitting
        current_dir = os.path.dirname(__file__)

        # 2. Dynamically climb up 3 levels to reach "Myntra Incentive Bleed Project"
        # ch05_cleaning.py -> chapters -> project_m -> project_m_reviewed -> Myntra Incentive Bleed Project
        base_dir = os.path.abspath(os.path.join(current_dir, "..", "..", ".."))

        # 3. Match your exact local path structure
        image_path = os.path.join(base_dir, "outputs", "operational_outlier_matrix.png")
        ...
        with col1:
            ...
            st.info(
                ...
                "While a typical customer spends ~12.5 mins daily, a distinct group stays logged in for 40 to 100+ mins.\n\n"
                "**The Business Reality:**\n"
                "These are not tech glitches; they are highly intentional, deal-seeking customers (**'Hunter'** segment). "
                "Because our wallet resets weekly, these users surge onto the app during final days to maximize engagement before balances expire.\n\n"
                "**📊 Impact:**\n"
        ...
        with col2:
            ...
            st.info(
                "**What the chart shows:**\n"
                "A massive, flat wall of data points stops abruptly at exactly 100 Rs.\n\n"
                "**The Business Reality:**\n"
                "Indicates that reward-distribution limits are functioning as intended. Our scratch-card campaign utilizes an exponential reward curve, "
                "but to protect margins, we enforce a hard cap of 100 Rs per card (`STATUS_QUO_CAP = 100`).\n\n"
                "**📊 Impact:**\n"
        ...
        with col3:
            ...
            st.info(
                "**What the chart shows:**\n"
                "Average order value sits around 773 Rs, but a natural trail of premium orders stretches up to 1,750 Rs.\n\n"
                "**The Business Reality:**\n"
                "A classic retail distribution reflecting healthy high-ticket purchases. Heavily driven by our **'Sure Thing'** cohort, "
                "who carry a much higher baseline order value (920 Rs) than casual browsers.\n\n"
                "**📊 Impact:**\n"
