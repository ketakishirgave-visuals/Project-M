...
# FIXED: Added explicit mapping for structural string alterations from the CSV recommendations
ACTION_TAG_COLORS = {
    "Invest": "#3FA66B",
    "Reduce": "#D4A24C",
    "Reduce -20%": "#D4A24C",
...
def render(data, skim_mode):
    ...
    for seg_id in range(6):
        ...
        action_color = ACTION_TAG_COLORS.get(action, "#7C8CA6")
        conf_level = SEGMENT_CONFIDENCE[seg_id]
        conf_icon, conf_color = BADGE_LABELS[conf_level]
        with cols[seg_id]:
            # FIXED: Updated precision to :.2f to accurately map 0.45% outlier pools without breaking layouts
            st.markdown(
                ...
                <div style="font-size:0.8rem; color:{ACCENT}; font-weight:600;">
                    {conf_row['population_share']*100:.2f}%
                </div>
    ...
    if not skim_mode:
        ...
        for seg_id in range(6):
            ...
            with st.expander(
                f"**{name}** — {conf_row['population_share']*100:.2f}% · "
                ...
                with right:
                    st.markdown(
                        f"""<div class="pm-card">
                        <b>Population</b><br>
                        {int(conf_row['population_count'])} users ({conf_row['population_share']*100:.2f}%)<br><br>
                        <b>Validation</b><br>
