...
FEATURES = {
    "Timing & Urgency": [
        ...
        ("j_curve_std", "Week-over-week variability in the j-curve ratio", "Stable vs. erratic urgency patterns — Aggressive Expiry Converters show high std"),
        ...
        ("scratch_velocity", "Fraction of scratches that happened on days 1–3 of the window", "Early scratchers tend to be organic; late scratchers are often expiry-driven"),
    ],
    "Engagement & Value": [
        ...
        ("conversion_rate", "Transactions divided by total sessions", "Session-to-purchase efficiency — distinguishes browsers from committed buyers"),
        ("session_density", "Sessions per active day", "Compressed, intense engagement vs. spread-out sporadic use"),
        ("late_session_intensity", "Mean sessions on days 6–7 of the window", "Captures the surge behavior unique to expiry-sensitive segments"),
    ],
    "Wallet Behavior": [
        ...
        ("expired_cashback_fraction", "Fraction of scratched cashback that was deliberately not redeemed", "The Hunter signal: deliberately letting small cards expire to save wallet cap for a bigger card"),
        ...
        ("full_price_rate", "Fraction of transactions with < ₹5 cashback applied", "High rate = organic buyer who doesn't need incentives to convert"),
    ],
}
...
def render(data, skim_mode):
    ...
    question_answer(
        ...
        answer="Twelve label-blind behavioral features — built without ever looking at the answer — capture timing, velocity, and value patterns unique to each customer.",
        badge_level="strong",
    )
