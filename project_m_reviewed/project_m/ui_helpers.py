...
def metric_spread(label: str, best_name: str, best_val: str, worst_name: str, worst_val: str, gap: str, unit_note: str = None):
    """
    Standard 'Best / Worst / Gap' output summary — used in place of a single
    isolated number wherever a block reports segment-level performance.
    Keeps every comparison legible as a range rather than one point estimate.
    """
    st.markdown(f'<div class="pm-evidence-label" style="margin-bottom:0.5rem;">{label.upper()}</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1:
        st.markdown(
            f'<div class="pm-card-dark" style="text-align:center; padding:0.9rem 0.6rem;">'
            f'<div style="font-size:0.7rem; color:#9A9CA3;">BEST</div>'
            f'<div style="font-size:1.3rem; font-weight:700; color:#3FA66B;">{best_val}</div>'
            f'<div style="font-size:0.76rem; color:#EDEBE6; margin-top:2px;">{best_name}</div></div>',
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            f'<div class="pm-card-dark" style="text-align:center; padding:0.9rem 0.6rem;">'
            f'<div style="font-size:0.7rem; color:#9A9CA3;">WORST</div>'
            f'<div style="font-size:1.3rem; font-weight:700; color:#C4573F;">{worst_val}</div>'
            f'<div style="font-size:0.76rem; color:#EDEBE6; margin-top:2px;">{worst_name}</div></div>',
            unsafe_allow_html=True,
        )
    with c3:
        st.markdown(
            f'<div class="pm-card-dark" style="text-align:center; padding:0.9rem 0.6rem;">'
            f'<div style="font-size:0.7rem; color:#9A9CA3;">GAP</div>'
            f'<div class="pm-metric-big" style="font-size:1.3rem;">{gap}</div>'
            f'<div style="font-size:0.76rem; color:#9A9CA3; margin-top:2px;">{unit_note or "best minus worst"}</div></div>',
            unsafe_allow_html=True,
        )


def sql_block(code: str, key: str, label: str = "Show SQL"):
    """
    Collapsible SQL presentation — keeps the query out of the main reading
    flow while keeping it one click away, with syntax highlighting intact.
    """
